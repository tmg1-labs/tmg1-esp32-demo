...
- 圧縮先バッファ `_frameBufferSize*2+64` がRice k=7で不足し `tmg1_mem_write` が黙って
  ...
  エラーを伝播しない点 (将来改善余地)。詳細 errors-log.md #7。
...
### 2026-06-28 セッション（仕様書 ↔ 実装の差異修正）

#### 作業内容
公開仕様書 `gitlab-profile/README.md` を実装(`lib/tmg1-codec`)に突き合わせて修正。
arduino の `.claude/architecture.md` の Range コーダ解説も実装に同期。

#### 修正したこと
- README: Version `0x01`→`0x02` / FileHeader Flags bit1 を reserved 化 /
  PredictionMethod ID を実装通り `1=Left, 2=Up` に訂正 /
  Range ペイロードを「行単位(行タイプビット+データ行のみ width ピクセル)」に訂正 /
  I-Frame の空行スキップを明記 / Overview の「C#」記述を Rust CLI+C++ codec へ更新。
- architecture.md: Range コーダを「32bit/4バイトフラッシュ」→「64bit/divide-first/8バイトフラッシュ」に訂正
  (式・正規化条件・初期化を [range_encoder.cpp]/[range_decoder.cpp] に一致)。
  ※ 過去の 2026-06-07 セッション2 の履歴エントリは当時の 32bit 記述のまま据え置き(履歴のため)。

#### 決定事項
| 決定 | 内容 | 理由 |
|---|---|---|
| FileHeader Flags bit1「delta default on」 | **未実装(reserved)** で確定 | v2 codec では最初から未配線。delta(I/P)は各 `FrameHeader.frameType` でフレーム単位に明示されるため、ファイル大域の「delタ既定」ヒントはデコーダが使わず冗長。delta 有無はエンコーダ設定(`EncodeConfig.deltaEnabled`)で完結し、ファイルに記録不要。旧 C# 版前提の placeholder が残っていただけ |

#### 注意点
- `gitlab-profile` は本ワークツリー外の別リポジトリ。コミット/プッシュは未実施(別途)。
- `architecture.md` 変更も未コミット。
- StartBit(FrameFlags bit0) はデコーダ対応済みだがエンコーダは現状フラグを立てない(仕様機能としては有効なので記述据え置き)。
