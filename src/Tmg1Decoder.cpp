...
bool Tmg1Decoder::begin(Stream& stream) {
  ...
  // Allocate buffers if not already allocated or if size changed (though video
  // size shouldn't change mid-stream)
  if (!_previousFrame) {
...
bool Tmg1Decoder::decodeFrame(uint8_t* buffer, size_t bufferSize) {
  ...
  if (frameHeader.payloadSize == 0) {
    if (frameHeader.frameType == 1) {  // P-Frame
      // If the output buffer is not the same as the previous frame buffer, copy
      // the content
  ...
  if (_stream->readBytes(reinterpret_cast<char*>(_payloadBuffer.data()), frameHeader.payloadSize) !=
  ...
  if (frameHeader.frameType == 1) {  // P-Frame
    ...
    if (decompressPayload(_payloadBuffer.data(), frameHeader.payloadSize, _tempFrame, _frameBufferSize,
                          frameHeader.frameFlags, frameHeader.frameType)) {
    ...
    if (decompressPayload(_payloadBuffer.data(), frameHeader.payloadSize, buffer, bufferSize, frameHeader.frameFlags,
...
uint16_t Tmg1Decoder::getHeight() const { return _fileHeader.height; }

uint16_t Tmg1Decoder::getTimebaseNum() const { return _fileHeader.timebaseNum; }

uint16_t Tmg1Decoder::getTimebaseDen() const { return _fileHeader.timebaseDen; }

uint32_t Tmg1Decoder::getLastPtsDelta() const { return _lastPtsDelta; }
...
bool Tmg1Decoder::decompressPayload(const uint8_t* src, size_t srcSize, uint8_t* dest, size_t destSize,
                                    uint8_t frameFlags, uint8_t frameType) {
...
bool Tmg1Decoder::decompressPayloadRange(const uint8_t* src, size_t srcSize, uint8_t* dest, size_t destSize,
                                         uint8_t frameFlags, uint8_t frameType) {
...
bool Tmg1Decoder::decompressPayloadRice(const uint8_t* src, size_t srcSize, uint8_t* dest, size_t destSize,
                                        uint8_t frameFlags, uint8_t frameType) {
  ...
  for (uint16_t y = 0; y < height; ++y) {
    ...
    while (bitsWrittenInLine < width) {
      if (reader.isEndOfStream()) {
        if (currentBit == 1) {
          printf(
              "ERROR: Stream ended unexpectedly on a white run at y=%d, bit "
              "%d.\n",
              y, bitsWrittenInLine);
          return false;
        }
